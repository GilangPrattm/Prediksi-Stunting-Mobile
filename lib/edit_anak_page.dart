...
class _EditAnakPageState extends State<EditAnakPage> {
  ...
  void initState() {
    ...
    _bbSekarangController.text = widget.dataAnak['berat_badan']?.toString() ?? '';
      ...
      builder: (context, child) {
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: const ColorScheme.light(primary: Color(0xFF2563EB), onPrimary: Colors.white, onSurface: Colors.black),
          ),
          child: child!,
        );
      },
  ...
  void _prosesEdit() async {
    if (_nikController.text.isEmpty || _namaAnakController.text.isEmpty || _namaOrtuController.text.isEmpty || _tglLahirController.text.isEmpty || _jenisKelamin == null) {
    ...
    if (sukses) {
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Data anak berhasil diperbarui!'), backgroundColor: Colors.green));
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Edit Data Anak', style: TextStyle(color: Colors.black, fontSize: 18, fontWeight: FontWeight.bold)),
        backgroundColor: Colors.white,
        elevation: 0,
            ...
            Container(
              ...
              child: const Row(
                children: [
                  ...
                  SizedBox(width: 10),
                  Expanded(child: Text('Perbarui informasi anak jika ada perubahan atau kesalahan ketik sebelumnya.', style: TextStyle(color: Colors.orange, fontSize: 13))),
                ],
              ),
            ),
  ...
  Widget _buildLabel(String text) {
    return Padding(padding: const EdgeInsets.only(bottom: 8.0, top: 15.0), child: Text(text, style: const TextStyle(fontWeight: FontWeight.w600, color: Color(0xFF475569))));
  }
  ...
  Widget _buildTextField(TextEditingController controller, String hint, TextInputType type) {
    return TextFormField(
      ...
      keyboardType: type,
      decoration: InputDecoration(hintText: hint, filled: true, fillColor: Colors.grey[50], border: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide.none)),
    );
  }
  ...
  Widget _buildDateTextField(TextEditingController controller, String hint, BuildContext context) {
    return TextFormField(
      ...
      decoration: InputDecoration(hintText: hint, filled: true, fillColor: Colors.grey[50], prefixIcon: const Icon(Icons.calendar_month, color: Color(0xFF2563EB)), border: OutlineInputBorder(borderRadius: BorderRadius.circular(15), borderSide: BorderSide.none)),
    );
  }
}
